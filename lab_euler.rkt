...
(define (get-vertices G)
 (cond ((null? G) '())
 (else (usort (append (car G) (get-vertices (cdr G)))))))
...
(define (degree2 L G)
  (if (null? G) L
      (degree2 (append L (list(car (car G))(car (cdr (car G)))))(cdr G))))

(define (degree G H)
 (degreeHelper H (degree2 '() G)))

(define (degreeHelper x y)
    (cond ((null? y) 0) ((equal? x (car y)) (+ 1 (degreeHelper x (cdr y))))
    (else (degreeHelper x (cdr y)))))
...
(define (all-even? G)
   (cond ((null? G) #t)
    (else (all-even2
    (map(lambda (x)(and (even? (degree G (car x)))(even? (degree G (cadr x)))))G)))))

(define (all-even2 K)
  (cond ((null? K) #t)
  (else (and (car K)(all-even2 (cdr K))))))




#|-------------------------------------------------------------------------------|
 |                         Part 2: Condition #2 (8 PTS)                          |
 |-------------------------------------------------------------------------------|#
...
;; 4 PTS
(define (adjacent G v)
  (cond ((null? G) '()) ((equal? v (car (car G))) (append (list (car (cdr (car G)))) (adjacent (cdr G) v)))
...
;; 4 PTS
(define (connected? G)
  (cond ((null? G) #t) (else (all-even2(map(lambda (x)
  (equal? (usort (dfs G x)) (get-vertices G))) (get-vertices G))))))




#|-------------------------------------------------------------------------------|
 |                          Part 3: The Finale (2 PTS)                           |
 |-------------------------------------------------------------------------------|#
