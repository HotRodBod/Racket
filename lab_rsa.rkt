...
;; Type signature: (mul-inv natural natural) -> natural
(define (mul-inv a b)
...
(define (mod-exp b e m)
  (cond
    ((and (> e 0) (= 0 (modulo e 2))) (modulo (expt (mod-exp b (/ e 2) m) 2) m) )
    ((= e 0) (modulo (modulo (expt b 0) m) m))
...
(define (encrypt plaintext key)
  (cond ((equal? "" plaintext) '())
        (else
         (cons
          (mod-exp (char->integer (car (string->list plaintext))) (cadr key) (car key))
          (encrypt (substring plaintext 1) key))))) 
...
;; 6 PTS
(define (translate ciphertext akey bkey)
  (cond
   ((null? ciphertext) '())
   (else (cons (mod-exp (car ciphertext) bkey akey) (translate (cdr ciphertext) akey bkey)))
   )
  )

(define (decrypt ciphertext key)
                (define bkey (cadr key))
                (define akey (car key))
  (define placeholder (translate ciphertext akey bkey))
  (define otherplaceholder (map integer->char placeholder))
                (list->string otherplaceholder))
