...
(define (compose2 a b)
  (if (null? b) '()
      (if (equal? (cadar b) (car a))
          (cons (list (caar b) (cadr a)) (compose2 a (cdr b)))
          (compose2 a (cdr b)))))
(define (compose S R) 
  (if (null? S) '()
      (make-set (append (compose2 (car S) R) (compose (cdr S) R) ))))
...
;; 5 PTS
(define (transitive2 R len)
  
  (if (= len 0) '()
      
      (union (power len R) (transitive2 R (- len 1)))))

(define (transitive-closure R)
  
  (if (null? R) '()
      
      (transitive2 R (length R)))) 
