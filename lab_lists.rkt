...
;; 2 PTS
(define (yoda words)
  (cons (car (reverse words)) (reverse(cdr (reverse words)))))
...
;; 2 PTS
(define (rotate c)
  (list c (list (car (reverse c)) (* -1 (car c))) (list (* (car c) -1) (* (car (cdr c)) -1)) (list (* (car (cdr c)) -1) (car c))))
...
;; 2 PTS
(define (get-state s)
 (car(cdr(car(reverse(car(cdr(reverse s))))))))
...
(define (combine-courses s1 s2)
 (append (car(reverse s1))(car(reverse s2))))