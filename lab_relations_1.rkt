...
(define (converse R)
  (if (equal? '() R)
        '()
        (append (list (reverse (car R))) (converse (cdr R)))))
...
(define (relate x R)
 (if (null? R)
      ...
      (if (equal? x (car (car R)))
          (cons (car (cdr (car R))) (relate x (cdr R)))
          (append '() (relate x (cdr R))))))
...
(define (scale X)
  (if (null? X)
      '()
      (append (cdr (car X)) (scale (cdr X)))))
  
             
            
(define (injective? R)
  (if (equal? (scale R) (make-set (scale R))) #true #false))
