...
;; 1 PTS
(define (divides? a b)
 (cond ((and (= a 0) (= b 0)) #t) ((= (modulo b a) 0) #t)  ((= a 0) #f)
        (else #f)))
...
;; 1 PTS
(define (lcm a b)
  (cond ((and (= a 0) (= b 0)) 0) (else (/ (* a b) (gcd a b)))))
...
(define (waterjug cA cB g)
  (Hwaterjug 0 0 cA cB g '()))

(define (Hwaterjug a b x y g helper)
  (cond
    ((= b g) (reverse (cons (list a b) helper)))
    ((= b y) (Hwaterjug a 0 x y g (cons (list a b) helper)))
    ((= a 0) (Hwaterjug x b x y g (cons (list a b) helper)))
    ((> (+ a b) y) (Hwaterjug (- a (- y b)) y x y g (cons (list a b) helper)))
    ((<= (+ a b) y) (Hwaterjug 0 (+ a b) x y g (cons (list a b) helper)))))
...
(define (Hfactorize n r)
  (cond ((= 0 (modulo n r)) (cons r (Hfactorize (/ n r)r))) ((= n 1) '())
        (else (Hfactorize n (+ r 1)))))

(define (factorize n)
  (cond ((= n 1) '(1)) ((= n 0) '(0)) (else (Hfactorize n 2))))
...
;; 2 PTS
(define (prime? n)
  (if (= n 0) #f (if (= 1 (length (factorize n))) #t #f)))
...
;; 3 PTS
(define (cong-mod? m)
  (define (cong-mod-helper r w)
    (if (= (modulo r m) (modulo w m)) #t #f)) cong-mod-helper)
...
#| Constructing "cong-mod?" in this way is useful because
 ...
 |
 | Now we can easily and repeatedly compare two numbers modulo 
 |  (cong-mod-3? 8 14).
 |
