...
;; 1 PTS
(define (pred p)
  (if (equal? z p) (error-predecessor) (car p)))
...
;; 2 PTS
(define (p+ a b)
  (if (and (pz? a) (pz? b)) a (if (pz? a) b (p+ (pred a) (succ b)))))
...
(define (p- a b)
  (cond
    ((and (pz? a) (pz? b)) (error-subtraction))
     (pz? b)
     (else
      (p- (pred a) (pred b))))
    (cond
      ((pz? b) a)
      (else (pred (p- a (pred b))))))
...
(define (p* a b)
  ...
  (if (or(pz? a) (pz? b)) z (if (pz? (pred b)) a (p+ a (p* a (pred b))))))
...
(define (primehelp p h)
  (cond
    ((p= h p) #t)
    ((p-divides? h p) #f)
    (else (primehelp p (succ h)))))
(define (p-prime? p)
  (cond
    ((pz? p) #f)
    ((pz? (pred p)) #f)
    (else (primehelp p (succ (succ z))))))